<template>
  <v-app>
    ...
    <v-main class="main-content">
      ...
      <div class="custom-container">
        ...
        <div class="viewer-panel">
          <PointCloudViewer ref="pointCloudViewer" @error="handleViewerError"
            @point-cloud-loaded="handlePointCloudLoaded" @create-object="handleCreateObjectFromMarker" />
          ...
          <div v-if="apiStore.isProcessing || apiStore.isAnalyzing || apiStore.isDownloading" class="loading-overlay">
            <v-progress-linear color="primary" height="25" striped v-model="apiStore.processingStep">
              <strong>{{ apiStore.processingStep }}%</strong>
            </v-progress-linear>

            <p class="mt-4">{{
              apiStore.isAnalyzing ? 'Analyzing objects...' : apiStore.processingMessage ? 'Processing: ' +
                apiStore.processingMessage : 'Processing...'
            }}</p>

            <!-- Add this progress indicator -->
            <v-progress-linear v-if="apiStore.processingProgress !== null" class="mt-2" color="primary" height="20"
        ...
        <div class="control-panel">
          <ControlPanel @file-uploaded="handleFileUploaded" @metadata-loaded="handleMetadataLoaded"
            @object-created="handleObjectCreated" @segmentation-run="handleSegmentationRun"
            @objects-analyzed="handleObjectsAnalyzed" @results-downloaded="handleResultsDownloaded" />
        </div>
      </div>
    </v-main>
    ...
    <v-dialog v-model="uiStore.showDescriptionDialog" max-width="600">
      <ObjectDescriptionCard :loading="false" :object="uiStore.selectedObjectForDescription" :show-actions="true"
        @close="uiStore.showDescriptionDialog = false" />
    </v-dialog>

    <!-- Object Analysis Dialog -->
    <ObjectAnalysisDialog v-model="uiStore.showAnalysisDialog" :loading="apiStore.isAnalyzing"
      :results="apiStore.analysisResults" @apply-label="applyAnalysisLabel"
      @apply-all="apiStore.applyAllAnalysisResults" @view-object="viewAnalyzedObject" />
  </v-app>
</template>
