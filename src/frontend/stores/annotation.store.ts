...
export const useAnnotationStore = defineStore('annotation', () => {
    ...
    const clickDataForApi = computed(() => {
        const clickIdx: Record<string, Record<string, number[]>> = { '0': {} };
        const clickTimeIdx: Record<string, Record<string, number[]>> = { '0': {} };
        const clickPositions: Record<string, Record<string, number[][]>> = { '0': {} };

        for (const click of clickedPoints.value) {
            const objKey = String(click.objectIdx);
            const partKey = String(click.partIdx ?? 0);
            ...
            // Initialize arrays if needed
            if (!clickIdx[objKey][partKey]) {
                clickIdx[objKey][partKey] = [];
                clickTimeIdx[objKey][partKey] = [];
                clickPositions[objKey][partKey] = [];
            }


            // Use the index if available, otherwise use -1
            clickIdx[objKey][partKey].push(click.index ?? -1);
            clickTimeIdx[objKey][partKey].push(click.timeIdx || 0);
            clickPositions[objKey][partKey].push(click.position);
        }
    ...
    const addClickPoint = (
        ...
        if (!fromUndo && !isUndoRedoOperation.value) {
            ...
            if (undoStack.value.length > MAX_UNDO_HISTORY) {
                const oldestAction = undoStack.value.shift();

                // Clean up resources from the oldest action if needed
                if (oldestAction?.markerId) {
                    removeMarker(oldestAction.markerId);
                }
            }
        }
    ...
    const applySelection = (
        ...
        // Skip if clicking too fast (except for undo operations)
        if (!fromUndo) {
            const now = Date.now();
            if (now - lastClickTime < MIN_CLICK_INTERVAL) {
                console.log('Selection throttled (too rapid)');
                return { count: 0, indices: [] };
            }
            lastClickTime = now;
        }
        ...
        let pointIndices = pointCloudStore.spatialIndex.findPointsInCube(
            position,
            uiStore.cubeSize
        );

        if (marker && pointIndices.length > 0 && pointCloudStore.pointCloudData.positions) {
            const positions = pointCloudStore.pointCloudData.positions;
            pointIndices = pointIndices.filter((idx) => {
                const i3 = idx * 3;
                const dx = positions[i3] - marker.position.x;
                const dy = positions[i3 + 1] - marker.position.y;
                const dz = positions[i3 + 2] - marker.position.z;
                return dx * dx + dy * dy + dz * dz <= marker.radius * marker.radius;
            })
        }

        console.log('pointIndices', pointIndices.length)
    ...
    const applyIndependentSelection = (
        position: number[],
        objectIdx: number,) => {
    ...
    const createMarker = (markerOptions: MarkerOptions): AnnotationMarker | null => {
        ...
        // const minRadius = uiStore.clickMode === 'point' ? 0.04 : 0.03;
        const minRadius = 0.03;

        const radius = markerOptions.radius || Math.max(minRadius, uiStore.cubeSize / 2);

        // Create unique ID for the marker
        const id = `marker-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
        // console.log(radius)
        // Create the marker in the scene
        const mesh = threeJsService.createMarkerSphere(position, color, radius, markerOptions.metadata);
    ...
    const removeMarker = (markerId: string): boolean => {
        ...
        if (marker.mesh) {
            ...
            const obj = scene?.children.find(c => c.uuid === marker.mesh?.uuid);
            if (obj) {
                scene?.remove(obj);
            }
