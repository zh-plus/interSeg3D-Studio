...
export const useApiStore = defineStore('api', () => {
    ...
    const processingStep = ref<number>(0);
    ...
    const handleApiError = (error: any, defaultMessage: string): string => {
        ...
        if (error.response) {
            ...
            if (typeof data === 'object' && data !== null) {
                errorMessage = `Server error ${error.response.status}: ${data.detail || data.message || JSON.stringify(data)
                    }`;
    ...
    // Actions
    const uploadPointCloud = async (file: File,
        ...
        try {
            await apiService.uploadPointCloud(file, (percent) => {
                onProgress?.(percent);
                processingStep.value = percent;
                console.log(processingStep.value)
            });
    ...
    const runSegmentation = async (): Promise<boolean> => {
        ...
        try {
            ...
            console.log('Click data for API:', clickData);
            ...
            // Check if there are any actual clicks
            const hasClicks = Object.keys(clickData.clickPositions).some(objKey => {
                const parts = clickData.clickPositions[objKey];
                return parts && Object.keys(parts).some(partKey => {
                    const positions = parts[partKey];
                    return positions && positions.length > 0;
                });
            });
            
            console.log('Has actual clicks:', hasClicks);
            console.log('ClickPositions structure:', JSON.stringify(clickData.clickPositions, null, 2));
            
            if (!hasClicks) {
                throw new Error('No clicks available. Please add clicks before running inference.');
            }
            ...
            const request: InferenceRequest = {
                ...
                // objectTypes: uiStore.objects.map(obj => obj.type),
                objectTypes: uiStore.objects.map(obj => obj.type || 'object').filter(t => t !== undefined),
            };

            console.log('Sending inference request:', {
                clickDataKeys: Object.keys(request.clickData),
                clickPositionsKeys: Object.keys(request.clickData.clickPositions),
                clickPositionsCount: Object.keys(request.clickData.clickPositions).length
            });
    ...
    const updateObjects = async (): Promise<boolean> => {
        ...
        operationLock.value = true;
        const clickData = annotationStore.clickDataForApi;
        // 为每个对象添加position属性，值来自clickData.clickPositions
        const objects = uiStore.objects.map((obj, index) => ({
            ...obj,
            // 如果有对应的点击位置则添加，否则可以设为null或默认值
            position: clickData.clickPositions[index + 1] || null
        }));

        try {
            const response = await apiService.updateObjects(objects);
    ...
    const downloadResults = async (): Promise<boolean> => {
        ...
        operationLock.value = true;
        processingMessage.value = 'Downloading results...';
        try {
            const response = await apiService.downloadResults((progress) => {
                processingStep.value = progress;
            });
            console.log('Blob size:', response);
