...
export const useUiStore = defineStore('ui', () => {
  ...
  const selectedPartIndex = ref<{objIdx: number, partIdx: number} | null>(null);
  ...
  const currentPartIdx = computed<{ objIdx: number; partIdx: number } | null>(() => {
    return selectedPartIndex.value != null
        ? { objIdx: selectedPartIndex.value.objIdx + 1, partIdx: selectedPartIndex.value.partIdx + 1 }
        : null;
  });
  ...
  const setClickMode = (mode: ClickMode): void => {
    ...
    console.log(selectedObjectIndex.value)
  };
  ...
  const selectPart = (objIdx: number, partIdx: number) => {
    selectedPartIndex.value = {objIdx, partIdx};
    clickMode.value = 'object';
  ...
  const createNewPart = (objectIndex: number): PartData | null => {
  if (objectIndex === null || objectIndex < 0 || objectIndex >= objects.value.length) return null;

  const obj = objects.value[objectIndex];
  if (!obj.parts) obj.parts = [];

  const newPartId = obj.parts.length + 1;
  const newPart: PartData = {
    id: newPartId,
    // name: partName.trim() || `Part ${newPartId}`
  };

  obj.parts.push(newPart);
