...
class ApiService {
     ...
     */
    async uploadPointCloud(file: File, onProgress: (progress: number) => void): Promise<AxiosResponse<any>> {
        ...
        pointCloudStore.fileSize = file.size;
    ...
    async runMaskObjectRecognition(mask: number[]): Promise<AxiosResponse<MaskObjectRecognitionResponse>> {
        ...
        try {
            return await api.post<MaskObjectRecognitionResponse>('/recognition/mask_obj_recognition', {
                mask: mask
            });
        } catch (error) {
     ...
     */
    async updateObjects(objects: { id: number; name: string; description?: string; type: string }[]): Promise<AxiosResponse<any>> {
        ...
        try {
            ...
            return await api.post('/recognition/update_objects', request);
        } catch (error) {
            ...
            return await api.get("/download/results", {
                ...
                onDownloadProgress: (progressEvent) => {
                    ...
                    const percentCompleted = Math.round(
                        (progressEvent.loaded * 100) / pointCloudStore.fileSize
                    );
