...
class ThreeJsService {
     ...
     */
    public createMarkerSphere(position: THREE.Vector3, color: THREE.Color, radius: number = 0.02, metadata?: any): THREE.Mesh | null {
        if (!this.context.scene) {
        ...
        try {
            ...
            if (metadata?.isRecrateMarker) {
            //     sphere.name = uiStore.objects[metadata.clickPointIndex].name
            // } else {
            //     sphere.name = uiStore.objects[uiStore.selectedObjectIndex!].name
                const objIdx = metadata.objectIdx ?? metadata.clickPointIndex;
                if (objIdx !== undefined && objIdx >= 0 && objIdx < uiStore.objects.length) {
                    sphere.name = uiStore.objects[objIdx].name;
                } else {
                    console.warn(`Invalid object index: ${objIdx}, using default name`);
                    sphere.name = 'click-sphere';
                }
            }
