<template>
  <div class="point-cloud-viewer">
    ...
    <ViewportComponent ref="viewportComponent" :interaction-mode="uiStore.interactionMode"
      :cursor-style="uiStore.cursorStyle" @scene-ready="handleSceneReady" @viewport-resize="handleViewportResize"
      @render-error="handleRenderError">
      <!-- Loading overlay -->
      <LoadingOverlay :message="pointCloudStore.isLoading ? 'Loading point cloud...' : 'Processing...'"
        :progress="pointCloudStore.loadingProgress ?? undefined"
        :show="pointCloudStore.isLoading || annotationStore.isProcessingSelection" />

      <!-- Mode indicator -->
      <ModeIndicator :click-mode="uiStore.clickMode" :mode="uiStore.interactionMode" />

      <!-- Selection info -->
      <SelectionInfo :can-redo="annotationStore.canRedo" :can-undo="annotationStore.canUndo"
        :click-count="annotationStore.clickCount" :click-points="annotationStore.clickedPoints"
        :coordinate="annotationStore.selectedCoordinate"
        :is-processing-selection="annotationStore.isProcessingSelection"
        :point-count="pointCloudStore.pointCloudData.pointCount" :show-debug="showDebug" />
    ...
    <ObjectEditPanel v-if="showObjectEditPanel" :selected-object="selectedObjectForEdit" :position="editPanelPosition"
      @close="showObjectEditPanel = false" @save="saveObjectChanges" />
  </div>
</template>
...
const selectObject = (event: MouseEvent): void => {
  ...
  let objId = segmentation[nearestPoint.index];
  if(objId === 0){
    objId = uiStore.objects.find(obj => obj.name === raycastResult.name)!.id;
  }
...
const handleAnnotationClick = (event: MouseEvent): void => {
  ...
  if (uiStore.clickMode === 'object' && uiStore.currentObjectIdx === null && !uiStore.currentPartIdx) {
  ...
  if (raycastResult && raycastResult.point) {
    ...
    if (nearestPoint) {
      ...
      let objectIdx = uiStore.clickMode === 'background' ? 0 : (uiStore.currentObjectIdx || 0);
      let partIdx = 0
      if(uiStore.currentObjectIdx === null && uiStore.currentPartIdx != null && uiStore.clickMode != 'background') {
        objectIdx = uiStore.currentPartIdx.objIdx
        partIdx = uiStore.currentPartIdx.partIdx
      }
      annotationStore.addClickPoint(nearestPoint.position, objectIdx,partIdx, nearestPoint.index, marker?.id);  // Pass the marker ID

      // Apply selection coloring
      // const extraArg = uiStore.clickMode === 'independent' ? marker : null;
      if(uiStore.clickMode === 'object'){
        annotationStore.applySelection(nearestPoint.position, objectIdx);
      }

      if (uiStore.selectedObjectIndex || uiStore.selectedObjectIndex == 0) {
        const obj = uiStore.objects[uiStore.selectedObjectIndex];
        if (obj) {
          obj.type = uiStore.clickMode;
        }
      }
