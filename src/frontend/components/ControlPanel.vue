<template>
  <div class="control-panel-container">
    ...
    <v-card class="mb-4 control-card" outlined>
      ...
      <v-card-text>
        <v-file-input :disabled="apiStore.isProcessing || apiStore.operationLock" accept=".ply"
          label="Upload Point Cloud (.ply)" prepend-icon="mdi-cloud-upload" @change="handleFileUpload"></v-file-input>

        <v-file-input :disabled="!pointCloudStore.hasPointCloud || apiStore.isProcessing || apiStore.operationLock"
          accept=".json" label="Load Metadata (.json)" prepend-icon="mdi-file-document-outline"
          @change="handleMetadataLoad"></v-file-input>
      </v-card-text>
    </v-card>
    ...
    <v-card class="mb-4" outlined>
      <v-card-title class="d-flex align-center">
        ...
        <SaveStatusIndicator :status="saveStatus" :last-saved="uiStore.lastSaveTime" />

        <v-spacer></v-spacer>

        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-btn :disabled="!uiStore.hasUnsavedChanges || apiStore.isProcessing || apiStore.operationLock"
              color="primary" icon v-bind="attrs" x-small @click="updateObjectInfo" v-on="on">
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              ...
              color="info" icon v-bind="attrs" x-small @click="analyzeObjects" v-on="on">
      ...
      <v-card-text>
        <v-text-field v-model="uiStore.newObjectName" :disabled="!pointCloudStore.pointCloudData.file"
          label="New Object Name" @keyup.enter="createNewObject"></v-text-field>
        <v-btn
          :disabled="!pointCloudStore.pointCloudData.file || !uiStore.newObjectName || apiStore.isProcessing || apiStore.operationLock"
          block color="primary" @click="createNewObject">
          Create Object
        </v-btn>
        <v-btn
          :disabled=" uiStore.selectedObjectIndex==null || apiStore.isProcessing || apiStore.operationLock || !pointCloudStore.pointCloudData.file"
          block
          color="secondary"
          @click="createNewPart"
      >
        ...
        <v-list v-if="uiStore.objects.length > 0" class="mt-4 object-list" dense>
          <v-list-item v-for="(object, i) in uiStore.objects" :key="i"
            :class="{ 'selected-object': uiStore.selectedObjectIndex === i }" :style="getObjectStyleClass(i + 1)"
            class="object-list-item" @click="selectObject(i)">
            <v-list-item-content>
              <v-list-item-title class="d-flex align-center justify-space-between">
                <span class="object-name">{{ object.name }}</span>
                <v-btn v-if="object.description" class="info-button" icon title="Show description" x-small
              ...
              <!-- Part List: always show, color same as object -->
        <v-list v-if="object.parts && object.parts.length > 0" dense>
          <v-list-item
            v-for="(part, j) in object.parts"
            :key="part.id"
            :class="{'selected-part': uiStore.selectedPartIndex?.objIdx === i && uiStore.selectedPartIndex?.partIdx === j}"
            :style="getPartStyleClass(i,j)"
            class="part-list-item"
            @click.stop="selectPart(i, j)"
          >
            <v-list-item-content>
              <v-list-item-title>
                Part {{ part.id }}
            ...
            <v-list-item-action v-if="uiStore.selectedPartIndex?.objIdx === i && uiStore.selectedPartIndex?.partIdx === j">
    ...
    <v-card class="mb-4" outlined>
      ...
      <v-card-text>
        <div class="d-block">
          <v-btn :disabled="!uiStore.currentObjectIdx" :color="clickMode === 'object' ? 'primary' : 'default'" block
            @click="clickMode = 'object'">
            <v-icon>mdi-shape</v-icon>
            Object
          </v-btn>

          <v-btn :color="clickMode === 'background' ? 'primary' : 'default'" block class="mt-2"
            @click="clickMode = 'background'">
            <v-icon>mdi-eraser</v-icon>
            Background
          </v-btn>

          <v-btn :color="clickMode === 'point' ? 'primary' : 'default'" block class="mt-2" @click="clickMode = 'point'">
    ...
    <v-btn
      ...
      :loading="apiStore.isProcessing && !apiStore.isAnalyzing" block class="mt-4" color="primary" x-large
    ...
    <v-btn
      :disabled="!pointCloudStore.segmentedPointCloud || apiStore.isProcessing || apiStore.isAnalyzing || apiStore.operationLock"
      :loading="apiStore.isAnalyzing" block class="mt-4" color="info" @click="analyzeObjects">
      ANALYZE OBJECTS
    </v-btn>

    <v-btn :disabled="!pointCloudStore.segmentedPointCloud || apiStore.isProcessing || apiStore.operationLock"
      :loading="apiStore.isDownloading" block class="mt-4" color="success" @click="downloadResults">
      SAVE RESULTS
    </v-btn>
  </div>
</template>
...
/**
 * Function to get style for part list items
 */
function getPartStyleClass(objId: number, partId: number) {
  // Generate a color based on the part index
  const isSelected = uiStore.selectedPartIndex?.objIdx === objId && uiStore.selectedPartIndex?.partIdx === partId;
  ...
  return {
    backgroundColor: getCssColorFromIndex(objId+1, parseInt(saturation), parseInt(lightness)),
    color: isSelected ? 'white' : 'black',
    cursor: 'pointer',
    fontWeight: isSelected ? 'bold' : 'normal',
  };
...
function selectObject(index: number) {
  ...
  uiStore.selectedPartIndex = null;
}
 ...
 * Select an object
 */
function selectPart(indexObj: number,indexPart: number) {
  uiStore.selectPart(indexObj,indexPart);
  uiStore.selectedObjectIndex = null;
}
...
function createNewPart() {
  if (uiStore.selectedObjectIndex === null || uiStore.selectedObjectIndex < 0) {
    alert('Please select an object first.');
    return;
  }
  // 这里假设 uiStore 有 createNewPart 方法
  const newPart = uiStore.createNewPart(uiStore.selectedObjectIndex);
  emit('part-created', newPart);
}
...
.part-list-item {
  border-bottom: none !important;  /* 去掉你自己加的 */
  cursor: pointer;
}
