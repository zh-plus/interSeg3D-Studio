...
export function useRaycasting(options?: UseRaycastingOptions) {
    ...
    const performRaycast = (threshold?: number): RaycastResult | null => {
        ...
        const MeshIntersects = context.raycaster.intersectObjects(context.scene?.children || []);

        PerformanceLoggerUtil.end('raycasting');

        if (MeshIntersects.length > 0) {
            for (let i = 0; i < MeshIntersects.length; i++) {
                const type = MeshIntersects[i].object.type
                if (type === 'Mesh') {
                    return {
                        point: MeshIntersects[i].point,
                        distance: MeshIntersects[i].distance,
                        name: MeshIntersects[i].object.name
                    }
                }
            }
        }
