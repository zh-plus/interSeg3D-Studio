...
class SetCriterion(nn.Module):
    ...
    def multiclass_dice_loss(self, input: Tensor, target: Tensor, eps: float = 1e-6,
        ...
        if check_target_validity:
            ...
            # print("torch.unique(class_ids):",torch.unique(class_ids))
    ...
    def loss_bce(self, outputs, targets, weights=None):
        # print(f"loss_bce outputs type: {type(outputs)}")
        pred_masks = outputs['part_predictions_mask']

        loss = 0.0

        for i in range(len(pred_masks)):
            # print("torch.unique(targets[i]):",torch.unique(targets[i]))
            # print("torch.unique(pred_masks[i]):",torch.unique(pred_masks[i]))
            # print("pred_masks[i].shape:",pred_masks[i].shape)
            # print("targets[i].shape:",targets[i].shape)
    ...
    def forward(self, outputs, targets, weights=None):
        ...
        if 'aux_outputs' in outputs:
            for i, aux in enumerate(outputs['aux_outputs']):
                for loss in self.losses:
                    l_dict = self.get_loss(loss, aux, targets, weights)
