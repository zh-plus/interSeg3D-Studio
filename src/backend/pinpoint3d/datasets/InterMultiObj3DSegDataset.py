...
class InterMultiObj3DSegDataset(Dataset):
    def __init__(self, scan_folder, scene_list, quantization_size, transforms=None):
        ...
        with open("/ssd1/yhj/Interactive-Partial-Segmentation/agile3d/data/scene_labels.json") as json_file:
            self.TF_labels = json.load(json_file)
    ...
    def __getitem__(self, i):
        ...
        num_obj = num_obj.split('_')[0]  # 只取第一个数字
        num_obj = int(num_obj)
        ...
        # 传入gt label的信息
        try:
            # pseudo_labels = self.TF_labels[scene_name]["gt_label"]
            pseudo_labels = self.TF_labels[scene_name]["pseudo_label"]
        except KeyError:
            # print(scene_name)
            pseudo_labels = []
        ...
        if data_sample:
            ...
            for obj_id, click_dict in click_idx_qv.items():
                if obj_id == '0':
                    continue
                for click_id, click_indices in click_dict.items():
                    if len(click_indices) > 0:
                        assert np.all(labels_qv[click_indices] == int(click_id)), "data sample not match!"
        else:
            click_idx_qv = {}


        return coords_qv, raw_coords_qv, feats_qv, labels_qv, labels_full_new, inverse_map, click_idx_qv, scene_name, num_obj, pseudo_labels 
...
def collation_fn(data_labels):
    coords, raw_coords, feats, labels, labels_full, inverse_map, click_idx, scene_name, num_obj, tf_label = list(zip(*data_labels)) 
    ...
    return coords_batch, raw_coords_batch, feats_batch, labels_batch, labels_full, inverse_map, click_idx, scene_name, num_obj, tf_label
