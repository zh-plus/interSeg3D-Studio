...
class InterSingleObj3DSegDataset(Dataset):
    def __init__(self, scan_folder, object_list, quantization_size, crop=False, transforms=None):
        ...
        with open(object_list) as json_file:
            self.data_samples = json.load(json_file)
        
        self.dataset_list = list(self.data_samples.keys())

        ''' 
        Example of dataset_list:
        array([
            ['scene0001', '1'],
            ['scene0001', '2'], 
            ['scene0002', '1'],
    ...
    #         colors_full = np.column_stack([point_cloud['red'], point_cloud['green'], point_cloud['blue']])/255
    ...
    def __getitem__(self, i):
            ...
            scene_name, num_part = sample_name.split('_obj_')
            object_id = int(self.data_samples[sample_name]['obj']['1'])//100
            # scene_name = self.dataset_list[i,0]
            # object_id = self.dataset_list[i,1]
            ...
            if self.crop:
                ...
                coords_full = np.column_stack([point_cloud['x'], point_cloud['y'], point_cloud['z']]).astype(np.float64)
                colors_full = np.column_stack([point_cloud['red'], point_cloud['green'], point_cloud['blue']])/255
                # labels_full = point_cloud['label'].astype(np.int32)//100
                labels_full = point_cloud['label'].astype(np.int32)
            else:
                point_cloud = read_ply(os.path.join(self.scan_folder, scene_name + '.ply'))
                coords_full = np.column_stack([point_cloud['x'], point_cloud['y'], point_cloud['z']]).astype(np.float64)
                colors_full = np.column_stack([point_cloud['red'], point_cloud['green'], point_cloud['blue']])/255
                labels_part = point_cloud['label'].astype(np.int32)
                labels_temp = point_cloud['label'].astype(np.int32)//100
                labels_full = (labels_temp == object_id).astype(np.int32)

                label_part = labels_part * labels_full  # 保留labels_part中对应labels_full=1的部分
                # labels_full = point_cloud['label'].astype(np.int32)
            ...
            return coords_qv, raw_coords_qv, feats_qv, labels_qv, labels_full, inverse_map, click_idx_qv, scene_name, object_id,label_part,coords_full,colors_full
...
def collation_fn(data_labels):
    coords, raw_coords, feats, labels, labels_full, inverse_map, click_idx, scene_name, object_id,label_part,coords_full,colors_full= list(zip(*data_labels)) 
    ...
    return coords_batch, raw_coords_batch, feats_batch, labels_batch, labels_full, inverse_map, click_idx, scene_name, object_id,label_part,coords_full,colors_full
