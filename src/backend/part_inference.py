...
class PartInference:
    ...
    def create_folders(self):
        Path(self.cfg.dataset.data_path).mkdir(exist_ok=True)
        Path(self.root).mkdir(exist_ok=True, parents=True)
        Path(self.dump_dir).mkdir(exist_ok=True)
    ...
    def solve_clustering(self, input_fname, uid, view_id, save_dir, out_render_fol, use_agglo=False, num_clusters=3,
                         is_pc=False, option=1, with_knn=True, export_mesh=True):
        print(uid, view_id)
        if not is_pc:
    ...
    def cluster(self, index, catagory):
        ...
        for model in selected:
            ...
            if labelId == index:
                return self.solve_clustering(fname, uid, view_id, save_dir=self.root, out_render_fol=self.dump_dir,
                                             use_agglo=self.use_agglo, num_clusters=catagory, is_pc=self.is_pc,
                                             option=self.option, with_knn=self.with_knn, export_mesh=self.export_mesh)
