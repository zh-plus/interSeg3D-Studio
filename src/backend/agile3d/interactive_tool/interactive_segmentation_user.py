...
class UserInteractiveSegmentationModel(abc.ABC):
    def __init__(self, device, config, dataloader_test):
        ...
        self.cube_size = self.config.cubeedge if hasattr(config,
                                                         "cubeedge") else 0.1  # the size of the segmentation mask for a single selected point
    ...
    def get_next_click(self, click_idx, click_time_idx, click_positions, num_clicks, run_model, gt_labels=None,
                       ori_coords=None, scene_name=None, **args):
        ...
        if run_model:
            ...
            if num_clicks == 0:
                ...
                print(f'click_idx: {self.click_idx}')
                print(f'click_time_idx: {click_time_idx}')
                ...
                line = now.strftime("%Y-%m-%d-%H-%M-%S") + '  ' + scene_name + '  NumObjects:' + str(
                    num_obj) + '  AvgNumClicks:' + str(round(num_click / num_obj, 1)) + '  mIoU:' + sample_iou + '\n'
                ...
                np.save(os.path.join(self.mask_folder, 'mask_' + str(round(num_click / num_obj, 1)) + '_' + sample_iou),
                        pred_full.cpu().numpy())
                np.save(
                    os.path.join(self.click_folder, 'click_' + str(round(num_click / num_obj, 1)) + '_' + sample_iou),
                    {
                        'click_idx': click_idx,
        ...
        self.visualizer.update_colors(
            colors=self.get_colors(reload_masks=False))  # self.object_mask is already up to date
        negative_semantic = self.object_mask[:, 2].copy() * 2  # negative semantic is 2, positive semantic is 1
        # as these channels are mutually exclusive, we get 0 for uncertain, 1 for positive and 2 for negative
        object_semantic = self.object_mask[:, 0] + negative_semantic
    ...
    def run_segmentation(self):  # Overrides standard run
        ...
        colors_full = np.asarray(
            self.points.vertex_colors if self.scene_point_type == "mesh" else self.points.colors).copy()
        ...
        self.pcd_features, self.aux, self.coordinates, self.pos_encodings_pcd = self.model.forward_backbone(data,
                                                                                                            raw_coordinates=self.raw_coords_qv)
        ### show UI
    ...
    def load_next_scene(self, quit=False, previous=False):
        ...
        if not previous and nxt:
            # load next scene
            self.scene_name, self.scene_point_type, self.points, labels_full_ori, record_file, mask_folder, click_folder, objects = next(
                self.dataloader_test)
            self.reset_masks()
        elif previous and prev:
            # load previous scene
            self.scene_name, self.scene_point_type, self.points, labels_full_ori, record_file, mask_folder, click_folder, objects = self.dataloader_test.load_scene(
                curr_scene_idx - 1)
            self.reset_masks()
        else:
            return
