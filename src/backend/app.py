    ...
    try:
        ...
        async with StepTimer("Initializing inference model"):
            current_inference = PointCloudInference(
                pretraining_weights='/ssd1/yhj/PartSeg/agile3d/weights/multi/checkpoint1099.pth',
                voxel_size=0.05
            )
        ...
        # # 缓存在推理对象里
        # current_inference.config.scene_center = center
        # current_inference.config.scene_diag = diag
        # current_inference.config.voxel_size = getattr(current_inference.config, "voxel_size", 0.05)
        # # +++++++++++++++++新增+++++++++++++++++
        ...
        #     "message": "File uploaded successfully",
        #     "filename": file.filename,
        #     "pointCount": int(len(coords)),
        #     "boundingBox": {"min": mins.tolist(), "max": maxs.tolist()},
        #     "sceneDiag": diag
        # })
...
async def run_inference(request: InferenceRequest):
    ...
    # # +++++++++++++++++新增+++++++++++++++++
    # diag = float(getattr(current_inference.config, "scene_diag", 1.0))
    # vx   = float(getattr(current_inference.config, "voxel_size", 0.05))

    # req_cube = float(request.cubeSize or 0.0)
    # if req_cube <= 0:
    #     req_cube = 0.01 * diag  # 默认=对角线1%

    # min_cs = max(0.5 * vx, 1e-4)
    # max_cs = 0.05 * diag
    # cube_size = req_cube
    # # cube_size = float(np.clip(req_cube, min_cs, max_cs))
    # # +++++++++++++++++新增+++++++++++++++++
    try:
        async with StepTimer("Converting click data"):
            # Convert click data to format expected by inference
            click_handler = ClickHandler()
            print(f"Received click data: {request.clickData}")
            ...
            for obj_idx_str, positions in request.clickData["clickPositions"].items():
                ...
                for i, pos in enumerate(positions):
                    ...
                    print(f"Click 实际使用的 cube_size: {click.cube_size}")
                    # print(f"Using cube_size: {cube_size}")
                    # print(f"voxel_size: {vx}, min_cs: {min_cs}, max_cs: {max_cs}")
...
async def get_point_cloud():
    """Get the current point cloud data with proper scaling"""
    global current_point_cloud, current_inference
    
    print("=== 收到 /api/pointcloud 请求 ===")
    
    if not current_point_cloud:
        print("没有点云数据")
        return JSONResponse(
            status_code=400,
            content={"message": "No point cloud loaded"}
        )
    
    print(f"当前点云: {len(current_point_cloud['coords'])} 个点")
    ...
    try:
        ...
        # 获取场景信息
        scene_diag = float(getattr(current_inference.config, "scene_diag", 1.0))
        print(f"场景对角线: {scene_diag}")
        
        # 计算合适的缩放因子
        if scene_diag > 1000:
            scale_factor = 100.0 / scene_diag
        elif scene_diag > 100:
            scale_factor = 50.0 / scene_diag
        else:
            scale_factor = 1.0
        ...
        # 扁平化数据
        xyz = scaled_coords.flatten().tolist()
        rgb = colors.flatten().tolist()
        
        print(f"返回数据: xyz长度={len(xyz)}, rgb长度={len(rgb)}")
        print("=== 请求处理完成 ===")
        ...
        return JSONResponse(content={
            ...
            "scaledBoundingBox": {
                "min": scaled_coords.min(axis=0).tolist(),
                "max": scaled_coords.max(axis=0).tolist()
            },
            "sceneDiag": scene_diag,
            "scaleFactor": scale_factor,
            "recommendedPointSize": calculate_recommended_point_size(scene_diag)
        })
        ...
        return JSONResponse(
            status_code=500,
            content={"message": f"Error processing point cloud: {str(e)}"}
        )

def calculate_recommended_point_size(scene_diag: float) -> float:
    """计算推荐的点大小"""
    if scene_diag > 1000:
        return 0.5
    elif scene_diag > 100:
